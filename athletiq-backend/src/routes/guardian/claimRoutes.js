...
router.post('/verify-claim', generalLimiter, async (req, res, next) => {
  try {
    ...
    if (!claimCode) {
  return sendResponse(res, { success: false, status: 400, message: 'Claim code is required' });
    }
...
router.post('/claim-by-details', generalLimiter, async (req, res, next) => {
  ...
  return sendResponse(res, { success: false, status: 400, message: 'School name, first name, last name, date of birth, and guardian phone are required' });
    }
  ...
  return sendResponse(res, { success: false, status: 400, message: 'Please provide a valid Nepali phone number' });
    }
...
router.post('/google-auth', generalLimiter, async (req, res, next) => {
  try {
    ...
    if (!googleToken) {
  return sendResponse(res, { success: false, status: 400, message: 'Google token is required' });
    }
