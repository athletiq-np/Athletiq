...
async function testRegistrationAndTeamOnboardingCorrected() {
  try {
    console.log('🚀 Testing Enhanced Registration & Team Onboarding System (Schema-Corrected)...\n');

    // Setup: Create test data with correct schema
    ...
    // Create test schools with proper schema
    const schoolsResult = await pool.query(`
      ...
      RETURNING *;
    `, [
      `SCH${timestamp}1`, `Test School Alpha ${timestamp}`, 'Test Address 1', 'alpha@test.com', 'Nepal', 'Kathmandu', 1,
      `SCH${timestamp}2`, `Test School Beta ${timestamp}`, 'Test Address 2', 'beta@test.com', 'Nepal', 'Pokhara', 1
    ]);
    
    const schools = schoolsResult.rows;
    console.log('✓ Test schools created:', schools.length);

    // Create test teams with correct schema (using sport_id)
    const footballSportId = 1; // Football from sports table
    const teamsResult = await pool.query(`
      ...
      RETURNING *;
    `, [
      `Alpha Football Team ${timestamp}`, schools[0].id, footballSportId, '2024-25',
      `Beta Football Team ${timestamp}`, schools[1].id, footballSportId, '2024-25'
    ]);
    
    const teams = teamsResult.rows;
    console.log('✓ Test teams created:', teams.length);

    // Create test players with correct schema
    ...
    // Create a simple eligibility check using correct column names
    ...
    for (const playerId of playerIds) {
      ...
      if (playerResult.rows.length > 0) {
        ...
        // Registration status check (using correct column)
    ...
    // Test 6: Get Tournament Teams (using correct column names)
    ...
    const team3Result = await pool.query(`
      ...
      VALUES ($1, $2, $3, $4)
      RETURNING *;
    `, [`Gamma Football Team ${timestamp}`, schools[0].id, footballSportId, '2024-25']);
    ...
    // Test 9: Audit Trail Verification (if table exists)
    ...
    try {
      ...
      console.log('⚠️  Audit trail table not available or empty');
    }
...
// Run the test
testRegistrationAndTeamOnboardingCorrected()
  .then((result) => {
    if (result.success) {
