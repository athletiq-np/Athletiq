...
export default function PlayerModal({ open, player, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md shadow-lg relative border border-gray-200 dark:border-gray-700">
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white text-lg"
        >
          ✕
        </button>
        ...
        <div className="flex items-center gap-4 mb-4">
          <img
            src={`${process.env.REACT_APP_API_URL || 'http://localhost:8000'}/uploads/${player.photo_url}`}
        ...
        <div className="mt-4">
          ...
          {player.birth_cert_url ? (
            <img
              src={`${process.env.REACT_APP_API_URL || 'http://localhost:8000'}/uploads/${player.birth_cert_url}`}
              alt="Birth Cert"
