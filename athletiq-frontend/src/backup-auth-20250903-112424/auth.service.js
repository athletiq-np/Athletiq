...
class AuthService {
  ...
  isAuthenticated() {
    try {
      ...
      const token1 = localStorage.getItem('athletiq_token');
      const token2 = localStorage.getItem(AUTH_CONFIG.TOKEN_KEY);
      const token3 = localStorage.getItem(AUTH_KEYS.TOKEN);
      
      console.debug('🔍 Token check details:', {
        'localStorage.athletiq_token': token1 ? 'EXISTS' : 'MISSING',
        'localStorage.auth_token': token2 ? 'EXISTS' : 'MISSING', 
        'localStorage.athletiq_token_2': token3 ? 'EXISTS' : 'MISSING',
        'localStorage length': localStorage.length,
        'all keys': Object.keys(localStorage).filter(key => key.includes('token') || key.includes('auth'))
      });
      
      const token = token1 || token2 || token3;
      
      if (!token) {
        console.debug('❌ No authentication token found in any location');
        return false;
      }
      
      console.debug('✅ Token found, checking validity...');
  ...
  clearAuthData() {
    console.warn('🧹 clearAuthData() called! Stack trace:', new Error().stack);
    ...
    try {
      ...
      console.debug('🗑️ Removing keys:', keysToRemove);
