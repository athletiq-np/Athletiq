...
const RoleBasedRouter = ({ children }) => {
  ...
  const getRoleBasedPath = (role, userType) => {
    ...
    // Map of role aliases to their canonical forms
    const roleAliases = {
    ...
    // Find the canonical role
    let canonicalRole = roleLower;
    for (const [canonical, aliases] of Object.entries(roleAliases)) {
      if (aliases.includes(roleLower)) {
        canonicalRole = canonical;
        break;
      }
    }

    // Map canonical roles to paths
    switch (canonicalRole) {
      ...
      default:
        console.warn('Unknown role, falling back to dashboard:', role);
        return '/dashboard';
    }
  };
