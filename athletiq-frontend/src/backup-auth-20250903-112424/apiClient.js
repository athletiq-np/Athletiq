...
apiClient.interceptors.response.use(
  ...
  const refreshToken = localStorage.getItem('athletiq_refresh_token') || localStorage.getItem(AUTH_CONFIG.REFRESH_TOKEN_KEY) || localStorage.getItem('refresh_token');
        if (!refreshToken) {
          clearAuthData();
          if (!window.location.pathname.includes('login')) {
            window.location.href = '/login?error=session_expired';
          }
          return Promise.reject(new ApiError('No refresh token available', ERROR_TYPES.AUTHENTICATION));
        }
        ...
        return Promise.reject(new ApiError('Invalid refresh response', ERROR_TYPES.AUTHENTICATION));
      } catch (refreshError) {
