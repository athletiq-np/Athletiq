...
export default function AthleteRegister() {
  ...
  useEffect(() => {
    async function loadSchools() {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:8000'}/api/schools`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL || 'http://localhost:8000'}/api/athletes`, data, {
        headers: {
