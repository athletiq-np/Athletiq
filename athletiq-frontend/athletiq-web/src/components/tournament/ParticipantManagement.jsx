...
const ParticipantManagement = ({ tournamentId, tournament }) => {
  ...
  const loadParticipants = async () => {
    ...
    try {
      ...
      setParticipants(activeTab === 'teams' ? mockTeams : mockAthletes);
    } catch (error) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <p className="font-medium">{team.athletes} registered</p>
        </div>
        <div>
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">{athlete.teamName}</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <div>
          ...
          <p className="font-medium">{athlete.age} • {athlete.grade}</p>
        </div>
        <div>
          <p className="text-sm text-gray-600">Games Played</p>
          <p className="font-medium">{athlete.stats.gamesPlayed}</p>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage teams and athletes for {tournament?.name}</p>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center space-x-2">
            ...
            <span>Add {activeTab === 'teams' ? 'Team' : 'Athlete'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Total {activeTab === 'teams' ? 'Teams' : 'Athletes'}</p>
              <p className="text-2xl font-bold text-gray-900">
                {activeTab === 'teams' ? mockTeams.length : mockAthletes.length}
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredParticipants.map(participant => (
            activeTab === 'teams' ? (
              ...
              <AthleteCard key={participant.id} athlete={participant} />
            )
          ))}
        </div>
      )}
      ...
      {filteredParticipants.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
            Add {activeTab === 'teams' ? 'Team' : 'Athlete'}
          </button>
        </div>
      )}
    </div>
