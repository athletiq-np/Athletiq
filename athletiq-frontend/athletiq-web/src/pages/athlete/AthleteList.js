...
export default function AthleteList() {
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= totalPages) {
      fetchAthletes(newPage, searchTerm);
    }
  };
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-4 mt-1">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                              {athlete.athlete_id || athlete.player_code || 'No Code'}
                            </span>
                            <span className="text-sm text-gray-500 dark:text-gray-400">
                              {athlete.school_name || 'No School'}
                            </span>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm text-gray-500 dark:text-gray-400">
                            DOB: {athlete.date_of_birth ? new Date(athlete.date_of_birth).toLocaleDateString() : 'Not specified'}
                          </div>
                          <div className="text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        ...
                        <div className="text-xs text-gray-400 dark:text-gray-500">
                          Registered: {new Date(athlete.created_at).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
