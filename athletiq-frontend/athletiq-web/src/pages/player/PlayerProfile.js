...
const PlayerProfile = () => {
  ...
  useEffect(() => {
    const fetchPlayer = async () => {
      try {
        // If no ID, try to get current player's profile
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
        <div className="max-w-4xl mx-auto">
          <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 rounded-lg p-6">
            <p className="text-yellow-800 dark:text-yellow-200">Player not found.</p>
          </div>
        </div>
      </div>
    );
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
              <div className="text-center">
                ...
                />
                <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{player.full_name}</h2>
                {player.full_name_nep && (
                  <p className="text-gray-600 dark:text-gray-400 mb-3">{player.full_name_nep}</p>
                )}
                ...
                <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg p-4 mb-4">
                  ...
                  <div className="text-2xl font-bold text-blue-800 dark:text-blue-300 font-mono">
                    {player.athlete_id || player.player_code || 'Not Assigned'}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">
                    {player.date_of_birth ? new Date(player.date_of_birth).toLocaleDateString() : 'Not specified'}
                  </p>
                </div>
                <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {player.height_cm && (
                    <div>
                      ...
                      <p className="text-gray-900 dark:text-white">{player.height_cm} cm</p>
                    </div>
                  )}
                  {player.weight_kg && (
                    <div>
                      <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Weight</label>
                      <p className="text-gray-900 dark:text-white">{player.weight_kg} kg</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">
                    {player.updated_at ? new Date(player.updated_at).toLocaleDateString() : 'Unknown'}
                  </p>
                </div>
              </div>
            </div>
