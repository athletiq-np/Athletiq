...
import { FaTrophy, FaMedal, FaPlus, FaCog } from 'react-icons/fa';
...
const KnockoutBracket = ({ 
  ...
  // Calculate positioning for matches
  const getMatchPosition = (roundIndex, matchIndex, totalMatches) => {
    const roundMultiplier = Math.pow(2, roundIndex);
    const spacing = 120 * roundMultiplier;
    const offset = (matchIndex * spacing) + (spacing / 2);
    return offset;
  };
      ...
      {roundIndex < totalRounds - 1 && (
        <div className="connection-line absolute top-1/2 -right-6 w-12 h-0.5 bg-gray-300 transform -translate-y-1/2"></div>
                  ...
                  <h3 className="text-lg font-semibold text-gray-800">
                    {rounds && rounds[roundIndex] ? rounds[roundIndex].name : `Round ${roundNum}`}
                  </h3>
                  <div className="text-sm text-gray-500">
                    {roundMatches.length} {roundMatches.length === 1 ? 'Match' : 'Matches'}
                  </div>
