...
Write-Host "Files that will be restored:" -ForegroundColor Yellow
git status --short

Write-Host "Proceeding with restore in 3 seconds..." -ForegroundColor Yellow
Start-Sleep -Seconds 3

# 3. Hard reset to last commit
...
if ([string]::IsNullOrEmpty($status)) {
    ...
    Write-Host "Some files still modified:" -ForegroundColor Yellow
    git status --short
}

Write-Host "Clean git restore complete!" -ForegroundColor Green
